...
class Elevator(object):
    def __init__(self):
        ...
        self.in_request = [0 for _ in range(20)]
        self.outside_request = [0 for _ in range(20)]
    ...
    def set_in(self, f):
        self.in_request[int(f-1)] = 1

    def set_outside(self, f, direction):
        if direction == 1:
            self.outside_request[int(f-1)] = 1
        elif direction == -1:
            self.outside_request[int(f-1)] = -1
    ...
    def check_in(self, floor):
        # if the direction of new in request is opposite of elevator direction,the request won't be responded
    ...
    def check_open(self):
        if self.in_request[int(self.current_floor - 1)] == 1:
            time.sleep(0.5)
            self.in_request[int(self.current_floor - 1)] = 0
        if abs(self.outside_request[int(self.current_floor - 1)]) == 1:
            time.sleep(0.5)
            self.outside_request[int(self.current_floor - 1)] = 0
...
class Controller(object):
    ...
    # flag:0: in request,1: outside request,ele_num: number of elevator requesting
    # floor: request floor,direction: direction of outside request
    def dispatch(self, flag, ele_num, floor, direction):
        # Internal request
        if flag == 0:
            if ele_num == 1:
                if self.elevator1.check_in(floor):
                    self.elevator1.set_in(floor)
                    self.elevator1.update_destination(floor)
                else:
                    print("Internal request error!")
            elif ele_num == 2:
                if self.elevator2.check_in(floor):
                    self.elevator2.set_in(floor)
                    self.elevator2.update_destination(floor)
                else:
                    print("Internal request error!")
            elif ele_num == 3:
                if self.elevator3.check_in(floor):
                    self.elevator3.set_in(floor)
                    self.elevator3.update_destination(floor)
                else:
                    print("Internal request error!")
            elif ele_num == 4:
                if self.elevator4.check_in(floor):
                    self.elevator4.set_in(floor)
                    self.elevator4.update_destination(floor)
                else:
                    print("Internal request error!")
            elif ele_num == 5:
                if self.elevator5.check_in(floor):
                    self.elevator5.set_in(floor)
    ...
    def find_nearest(self, floor, direction):
        ...
        if min_dis == 100:
            ...
            if min_index == 1:
                self.elevator1.set_outside(floor, direction)
            elif min_index == 2:
                self.elevator2.set_outside(floor, direction)
            elif min_index == 3:
                self.elevator3.set_outside(floor, direction)
            elif min_index == 4:
                self.elevator4.set_outside(floor, direction)
            elif min_index == 5:
                self.elevator5.set_outside(floor, direction)
            return True
